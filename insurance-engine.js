...
var InsuranceEngine = (function() {
  ...
  //  一、产品数据库（可独立维护，定期更新）
  //  格式说明：
  //    prices: [{ ageMin, ageMax, price }] 按年龄阶梯的年保费
  //    healthStrictness: 1=宽松 2=中等 3=严格(需体检)
  //    needSocialIns: 是否需要医保才能享受该费率
  //    guaranteeYears: 保证续保年数（null=不保证）
  //    features[]: 产品特色标签
  //    active: 是否在售
  // ==========================================================
  ...
  var SOCIAL_INSURANCE_MAP = {
    "深圳一档": { city: "深圳", type: "职工", huiminId: "hm_shenzhen_2026" },
    "深圳二档": { city: "深圳", type: "职工", huiminId: "hm_shenzhen_2026" },
    "广州职工": { city: "广州", type: "职工", huiminId: "hm_guangzhou_2026" },
    "广州灵活就业": { city: "广州", type: "灵活就业", huiminId: "hm_guangzhou_2026" },
    "汕尾居民": { city: "汕尾", type: "居民", huiminId: "hm_shanwei_2026" },
    "汕尾城乡": { city: "汕尾", type: "居民", huiminId: "hm_shanwei_2026" },
    "深圳灵活就业": { city: "深圳", type: "灵活就业", huiminId: "hm_shenzhen_2026" },
    "无医保": null, "零医保": null, "无": null, "军队体系": { city: "军队", type: "体系内", huiminId: null },
    "": null
  };
  ...
  var COVERAGE_REQUIREMENTS = [
    ...
    { type: "惠民保", condition: function(m) { var si = SOCIAL_INSURANCE_MAP[m.医保]; return si && si.huiminId; }, desc: "政府指导的低价补充保险" },
    { type: "重疾险", condition: function(m) { return m.age >= 18 && m.age <= 50; }, desc: "确诊重疾后的收入补偿", priority: "P1" },
    { type: "定寿", condition: function(m) { var debt = parseInt(m.负债) || 0; return (m.role === "户主" || m.role === "配偶") && debt > 0; }, desc: "家庭支柱身故后的债务覆盖", priority: "P0" },
  ...
  function getSocialInfo(insuranceType) {
    return SOCIAL_INSURANCE_MAP[insuranceType] || null;
  }
  ...
  function canInsure(product, member) {
    ...
    // 社保要求
    if (product.needSocialIns && (!member.医保 || member.医保 === "无" || member.医保.indexOf("零医保") > -1)) {
      return { ok: false, reason: "需要医保才能按社保费率投保" };
    }
    if (product.regionRestrict) {
      var si = getSocialInfo(member.医保);
      ...
      COVERAGE_REQUIREMENTS.forEach(function(req) {
        ...
        if (existingPolicy) {
          ...
          if ((req.type === "百万医疗" || req.type === "重疾险") && existingPolicy.guaranteedYears &&
              (existingPolicy.guaranteedYears.indexOf("1年不保证") > -1 || existingPolicy.guaranteedYears === "1年期")) {
            results.gaps.push({
