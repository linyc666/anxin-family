...
var PolicyExtractor = (function() {
  ...
  var EXTRACTION_PROMPT = [
    '你是一个保险单证数据提取助手。请从以下OCR识别出的保单文字中，提取结构化信息。',
    '',
    '## 提取字段（能提取多少提取多少，不确定的留空）：',
    '- policyName: 保单/产品名称（如"太保互联网个人长期B款医疗保险"）',
    '- company: 保险公司（如"太平洋健康""人保健康"）',
    '- type: 保险类型（百万医疗/意外险/重疾险/定寿/防癌险/惠民保/车险/家财险/其他）',
    '- premium: 年缴保费金额，只输出数字（如 443）',
    '- coverage: 保额/保障额度描述（如"一般200万·重疾400万"）',
    '- deductible: 免赔额描述（如"一般1万·重疾0免赔"）',
    '- effectiveDate: 生效日期，格式YYYY-MM-DD',
    '- expiryDate: 到期日期，格式YYYY-MM-DD（终身或长期续保的留空）',
    '- guaranteeYears: 续保条件（终身保证续保/20年保证续保/6年保证续保/1年期/1年不保证续保）',
    '- insuredName: 被保险人姓名',
    '- policyNo: 保单号',
  ...
  function regexExtract(ocrText) {
    ...
    // 续保条件
    if (/终身保证续保|终身续保/g.test(text)) result.guaranteeYears = '终身保证续保';
    else if (/20年.*续保|保证续保.*20年/g.test(text)) result.guaranteeYears = '20年保证续保';
    else if (/6年.*续保|保证续保.*6年/g.test(text)) result.guaranteeYears = '6年保证续保';
    else if (/1年.*续保|不保证续保/g.test(text)) result.guaranteeYears = '1年不保证续保';
